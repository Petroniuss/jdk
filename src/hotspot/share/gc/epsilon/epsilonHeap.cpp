...
jint EpsilonHeap::initialize() {
  ...
  // Initialize marking bitmap, but not commit yet
  size_t bitmap_page_size = UseLargePages ? os::large_page_size() : (size_t)os::vm_page_size();
  size_t _bitmap_size = MarkBitMap::compute_size(heap_rs.size());
  size_t bitmap_size  = align_up(_bitmap_size, bitmap_page_size);
...
//  Universe::oops_do(cl); // OopsStorage
//  Management::oops_do(cl); // OopsStorage
